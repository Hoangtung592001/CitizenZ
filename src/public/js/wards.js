...
.then(data => {
    data.forEach((unit, index) => {
        addProvidedAccount(index + 1, unit.name, unit.ward_id);
    })
});
...
var defaultValueRegister = $('.select-name-register').find(":selected").attr("data-id");
...
fetch("http://localhost:5000/get_info/analyse_population", {
        ...
        body: JSON.stringify({
            id: window.location.href.slice(22, 26)
        })
    })
    .then(data => data.json())
    .then(data => {
